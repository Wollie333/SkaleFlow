...
export function ControlsBar({
  ...
  onToggleRecording, onFlag, onEndCall,
}: ControlsBarProps) {
  if (!callActive) return null;

  return (
    <div className="flex items-center justify-center gap-3 px-4 py-3 bg-[#0F1F1D] border-t border-white/10">
      {/* Mic toggle */}
      <button
      ...
      <button
        onClick={onEndCall}
        className="px-5 py-3 rounded-full bg-red-500 hover:bg-red-600 text-white font-medium transition-colors"
      >
        End Call
      </button>
    </div>
