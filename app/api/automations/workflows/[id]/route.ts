...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existing } = await supabase.from('automation_workflows').select('organization_id').eq('id', id).single();
  ...
  const { data, error } = await supabase.from('automation_workflows').update(updateData).eq('id', id).select().single();
