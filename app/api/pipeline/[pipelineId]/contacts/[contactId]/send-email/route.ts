...
export async function POST(request: NextRequest, { params }: { params: { pipelineId: string; contactId: string } }) {
  ...
  const { data: contact } = await supabase.from('pipeline_contacts').select('email, organization_id').eq('id', params.contactId).single();
  ...
  const context = await buildMergeContext(params.contactId);
