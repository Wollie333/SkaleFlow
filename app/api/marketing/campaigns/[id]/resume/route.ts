...
export async function POST(
  ...
  if (!member || !['owner', 'admin'].includes(member.role)) {
    return NextResponse.json({ error: 'Only owners and admins can resume campaigns' }, { status: 403 });
  }
  ...
  try {
    ...
    await adapter.updateCampaignStatus(accountData.tokens, campaign.platform_campaign_id, 'ACTIVE');
