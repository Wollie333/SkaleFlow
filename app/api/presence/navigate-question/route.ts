...
import { getPresencePhaseByNumber } from '@/config/presence-phases';
...
export async function POST(request: Request) {
  try {
    ...
    // Clear conversation for fresh thread
    await supabase
    ...
    return NextResponse.json({ success: true, questionIndex: targetQuestionIndex });
  } catch (error) {
    console.error('Presence navigate question error:', error);
    return NextResponse.json({ error: 'Failed to navigate question' }, { status: 500 });
  }
}
