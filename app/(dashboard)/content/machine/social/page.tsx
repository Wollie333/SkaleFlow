...
export default function SocialPostsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (!user) {
          console.error('[SOCIAL] No authenticated user found');
          setIsLoading(false);
          return;
        }
        ...
        if (membership?.organization_id) {
          ...
          console.error('[SOCIAL] No org membership found for user:', user.id);
        }
      } catch (err) {
        console.error('[SOCIAL] loadData error:', err);
  ...
  const toggleBrandVar = (key: string) => {
    setSelectedBrandVars(prev => {
      ...
      // If already at max, ignore the add (don't remove existing)
      return next;
    });
  };
  ...
  // ─── Generation ─────────────────────────────────────────────────────
  // showGenModal = true immediately on click; batchId arrives a moment later
  const [showGenModal, setShowGenModal] = useState(false);

  const handleGenerate = async () => {
    if (!organizationId || placementCount === 0 || !effectiveModelId) {
      console.warn('[SOCIAL] handleGenerate bailed:', { organizationId, placementCount, effectiveModelId });
      return;
    }

    console.log('[SOCIAL] handleGenerate starting:', { organizationId, placementCount, effectiveModelId });

    // Show modal IMMEDIATELY — before any API calls
    ...
    // Mark all placements as generating
    ...
    try {
      ...
      // Create one content item per placement
      for (const { platform, placement } of allSelectedPlacements) {
        ...
        console.log(`[SOCIAL] Creating content item: platform=${platform}, placement=${placement}, format=${format}`);
        ...
        if (createData.item?.id) {
          ...
          console.log(`[SOCIAL] Created item: ${createData.item.id} → ${placement}`);
        } else {
          console.warn('[SOCIAL] No item.id in response:', createData);
        }
      }

      if (createdIds.length === 0) throw new Error('No content items were created');

      // Save the ID -> placement mapping
      ...
      console.log(`[SOCIAL] Mapped ${createdIds.length} items to placements`);

      // Enqueue batch
      console.log(`[SOCIAL] Enqueueing batch: ${createdIds.length} items, model=${effectiveModelId}`);
      const queueRes = await fetch('/api/content/generate/queue', {
        ...
        body: JSON.stringify({
          ...
          // null = smart mode (AI picks 7 random per post); otherwise use user-selected vars
          selectedBrandVariables: selectedBrandVars.size > 0 ? Array.from(selectedBrandVars) : null,
          // null = smart mode (AI picks best template); otherwise use user-selected overrides
      ...
      if (queueData.batchId) {
        console.log(`[SOCIAL] Batch enqueued: ${queueData.batchId}, totalItems=${queueData.totalItems}`);
  ...
  const handleBatchComplete = useCallback(async () => {
    ...
    if (items && items.length > 0) {
      // Store calendar_id for "View Calendar" navigation
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {/* Generation tracker is now in a modal — see bottom of JSX */}
        </div>
              ...
              {placementCount > 0 && (
                <div className="bg-stone-50 rounded-lg px-3 py-2 mt-3">
                  <p className="text-xs font-medium text-teal">
                    {enabledPlatforms.length} channel{enabledPlatforms.length !== 1 ? 's' : ''}, {placementCount} post{placementCount !== 1 ? 's' : ''}
                  </p>
                </div>
              )}
            </Card>
